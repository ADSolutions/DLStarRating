...
#pragma mark - UIImageManipulation
...
- (void)centerIn:(CGRect)_frame with:(NSInteger)numberOfStars
{
    ...
    CGFloat aspect = CGRectGetHeight(_frame) <= CGRectGetWidth(_frame) ? CGRectGetHeight(_frame) : CGRectGetWidth(_frame);
    
	self.frame = CGRectMake((size.width*self.tag) + gapToApply, newY, aspect, aspect);
}
